...
- Fix: ログアウトしてもヒントや案内の既読状態が消えないように
- Enhance: 特定のアカウントからのログアウト時に、そのアカウントのデータのみが確実に削除されるように
- Enhance: 設定のクラウド同期とバックアップの保存先アカウントを固定できるように（アカウントを切り替えても保存先が変わらなくなります）
