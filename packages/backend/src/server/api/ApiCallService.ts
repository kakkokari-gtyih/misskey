...
export class ApiCallService implements OnApplicationShutdown {
	...
	public handleRequest(
		...
		return this.telemetryService.startSpan('API: ' + endpoint.name, () => this.authenticateService.authenticate(token).then(async (aRes) => {
			...
			const call = this.call(endpoint, user, aRes.accessToken, aRes.sudo, body, null, request).then((res) => {
	...
	public async handleMultipartRequest(
		...
		try {
			...
			return await this.telemetryService.startSpan('API: ' + endpoint.name, () => this.authenticateService.authenticate(token).then(async (aRes) => {
				let user: MiJwtUser | MiLocalUser | null = aRes.user;
				if (endpoint.meta.requireFullUserData && user) {
					user = (await this.cacheService.localUserByIdCache.fetchMaybe(user.id, () => this.usersRepository.findOneBy({ id: user!.id }).then(x => x ?? undefined) as Promise<MiLocalUser | undefined>)) ?? null;
				}

				const call = this.call(endpoint, user, aRes.accessToken, aRes.sudo, fields, {
